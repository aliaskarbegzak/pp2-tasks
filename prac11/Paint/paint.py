...
def calculate_square(start, end): 
    ...
    left = min(x1, x2) 
    top = min(y1, y2) 
    walls = min(abs(x2 - x1), abs(y2 - y1)) 
 
    return pygame.Rect(left, top, walls, walls) 
...
while running: 
        ...
        if event.type == pygame.QUIT: 
            ...
            if event.button == 1: 
                if drawing: 
                    ...
                    if tool in ("rect", "circle", "rhombus", "equilateral_triangle", "right_triangle"): 
                        finalize_shape() 
    ...
    if drawing and tool in ("rect", "circle", "rhombus", "equilateral_triangle", "right_triangle") and start_pos and current_pos: 
        draw_color = get_draw_color() 
